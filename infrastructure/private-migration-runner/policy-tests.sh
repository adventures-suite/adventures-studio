...
require 'runner-group: ${{ steps.validate-runner-group.outputs.runner-group }}' "$workflow"
require "'runner-group=private-sql-migration-vnet' >> \"\$GITHUB_OUTPUT\"" "$workflow"
require "if: needs.readiness-guard.result == 'success' && needs.readiness-guard.outputs.runner-group == 'private-sql-migration-vnet'" "$workflow"
require 'group: ${{ needs.readiness-guard.outputs.runner-group }}' "$workflow"
reject 'group: ${{ vars.' "$workflow"
