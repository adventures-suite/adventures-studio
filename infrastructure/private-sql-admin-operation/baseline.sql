...
SELECT grantee.name AS grantee, permissions.state_desc,
       ...
       CASE permissions.class
         WHEN 0 THEN DB_NAME()
         WHEN 1 THEN CASE WHEN OBJECT_SCHEMA_NAME(permissions.major_id) IN (N'dbo', N'planning', N'auth', N'audit') AND OBJECT_NAME(permissions.major_id) IN (N'AdventuresSuiteSchemaVersions', N'AuditEvents', N'CompanionInformationPolicyAssignmentEvents', N'CreatorMembershipPermissionGrants', N'CreatorMembershipRoles', N'CreatorMemberships', N'ExternalIdentities', N'UserSessions', N'Users', N'Accommodations', N'AdventurePlanCreateResults', N'AdventurePlans', N'BudgetItems', N'CompanionInformationPolicyAssignments', N'DestinationVisits', N'ItineraryDays', N'PackingItems', N'PlannedActivities', N'PlanningNotes', N'PlanningTasks', N'Reservations', N'TransportationSegments', N'TravelerParticipations', N'TravelerPreferences', N'Travelers') THEN QUOTENAME(OBJECT_SCHEMA_NAME(permissions.major_id)) + N'.' + QUOTENAME(OBJECT_NAME(permissions.major_id)) ELSE N'unexpected-redacted' END
