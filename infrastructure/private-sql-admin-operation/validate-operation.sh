...
[[ "$ADMINISTRATOR_IDENTITY_RESOURCE_ID" == "/subscriptions/$subscription/resourceGroups/rg-adventures-suite-dev/providers/Microsoft.ManagedIdentity/userAssignedIdentities/id-adventures-suite-sql-bootstrap-dev" ]] || exit 1
[[ "$MIGRATION_IDENTITY_RESOURCE_ID" == "/subscriptions/$subscription/resourceGroups/rg-adventures-suite-dev/providers/Microsoft.ManagedIdentity/userAssignedIdentities/id-adventures-suite-migrate-job-dev" ]] || exit 1
...
[[ "$SQL_SERVER_RESOURCE_ID" == "/subscriptions/$subscription/resourceGroups/rg-adventures-suite-dev/providers/Microsoft.Sql/servers/adventures-suite-dev-sql" ]] || exit 1
[[ "$SQL_DATABASE_NAME" == AdventuresSuiteDevelopment ]] || exit 1
[[ "$SQL_PRIVATE_ENDPOINT_RESOURCE_ID" == "/subscriptions/$subscription/resourceGroups/rg-adventures-suite-dev/providers/Microsoft.Network/privateEndpoints/pe-adventures-suite-dev-sql" ]] || exit 1
case "$OPERATION_MODE" in
  baseline) [[ -z "${OPERATION_APPROVAL_SHA256:-}" ]] || exit 1 ;;
  bootstrap|cleanup|denial-proof) [[ "${OPERATION_APPROVAL_SHA256:-}" =~ ^[0-9a-f]{64}$ ]] || exit 1 ;;
  *) exit 1 ;;
esac
