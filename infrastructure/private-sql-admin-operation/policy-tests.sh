...
jq -e '.additionalProperties==false and (.properties.outcome.enum|index("At0012"))!=null and (.properties.outcome.enum|index("At0009"))!=null and .properties.permissions.maxItems==168 and .properties.principals.maxItems==1 and .properties.principals.items.properties.name.const=="AdventuresSuiteMigrationDev-ffc9a" and .properties.journal.properties.scripts.maxItems==13 and .properties.roles.maxItems==5 and .properties.residue.properties.resourceCount.const==0' "$dir/evidence.schema.json" >/dev/null
...
jq -e '.deadlineMinutes==30 and .automaticRetryCount==0 and .baselineMustPrecedeBootstrap==true and .baselineCanInvokeBootstrap==false and .successfulBaselineOutcomes==["complete","At0012","At0009","At0006","absent"] and .cleanup.independent==true and (.operations==["baseline","bootstrap","bootstrap-policy-role","bootstrap-initial-owner","bind-application-planning-runtime","cleanup","denial-proof"]) and (.failureOutcomes|index("ambiguous"))!=null and (.failureOutcomes|index("cleanupPartial"))!=null and (.authorityBoundaries|length)==4' "$dir/operation-policy.json" >/dev/null
...
if validate OPERATION_MODE=bootstrap-initial-owner OPERATION_APPROVAL_SHA256="$digest" SUPPORT_ID=support-0001 CORRELATION_ID=correlation-0001; then
  echo "initial-owner bootstrap lacked exact user identity" >&2; exit 1
fi
validate OPERATION_MODE=bootstrap-initial-owner OPERATION_APPROVAL_SHA256="$digest" SUPPORT_ID=support-0001 CORRELATION_ID=correlation-0001 TARGET_USER_ID=user_development_owner
for invalid in 'TARGET_USER_ID=User_Invalid' 'TARGET_USER_ID=user-hyphenated' 'TARGET_USER_ID=ab' 'SUPPORT_ID=bad' 'CORRELATION_ID=case altered'; do
  if validate OPERATION_MODE=bootstrap-initial-owner OPERATION_APPROVAL_SHA256="$digest" SUPPORT_ID=support-0001 CORRELATION_ID=correlation-0001 TARGET_USER_ID=user_development_owner "$invalid"; then
    echo "invalid initial-owner binding accepted" >&2; exit 1
  fi
done
...
require 'InitialOwnerCreatorId = "creator_tsa_01"' "$root/src/AdventuresSuite.DatabaseMigrator/SqlAdministratorOperationRunner.cs"
require 'InitialOwnerMembershipId = "membership_tsa_initial_owner"' "$root/src/AdventuresSuite.DatabaseMigrator/SqlAdministratorOperationRunner.cs"
...
reject 'Environment.GetEnvironmentVariable("ADVENTURESSUITE_INITIAL_OWNER_CREATOR_ID")' "$root/src/AdventuresSuite.DatabaseMigrator/SqlAdministratorOperationRunner.cs"
