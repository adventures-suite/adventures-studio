...
jq -e '.deadlineMinutes==30 and .automaticRetryCount==0 and .baselineMustPrecedeBootstrap==true and .baselineCanInvokeBootstrap==false and .cleanup.independent==true and (.operations==["baseline","bootstrap","bootstrap-policy-role","bootstrap-initial-owner","cleanup","denial-proof"]) and (.failureOutcomes|index("ambiguous"))!=null and (.failureOutcomes|index("cleanupPartial"))!=null and (.authorityBoundaries|length)==4' "$dir/operation-policy.json" >/dev/null
