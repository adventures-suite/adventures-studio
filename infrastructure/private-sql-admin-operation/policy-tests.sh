...
jq -e '.additionalProperties==false and .properties.permissions.maxItems==164 and .properties.principals.maxItems==1 and .properties.principals.items.properties.name.const=="AdventuresSuiteMigrationDev-ffc9a" and .properties.journal.properties.scripts.maxItems==11 and .properties.roles.maxItems==5 and .properties.residue.properties.resourceCount.const==0' "$dir/evidence.schema.json" >/dev/null
