...
/// <summary>Protects accommodation authorization, validation, concurrency, and audit behavior.</summary>
public sealed class AccommodationAddServiceTests
{
    ...
    /// <summary>An authorized request commits a Proposed accommodation and matching audit.</summary>
    [Fact]
    public async Task AddAsync_AuthorizedRequest_CommitsAccommodationAndAudit()
    {
        var transaction = new RecordingTransaction(Creator, Plan());
        var result = await Service(transaction).AddAsync(Command());
        Assert.Equal(AddAccommodationOutcome.Added, result.Outcome);
        Assert.Equal(2, result.Version);
        Assert.True(transaction.Committed);
        Assert.Equal(PlanItemStatus.Proposed, transaction.Repository.Accommodation?.Status);
        Assert.Equal("Europe/Rome", transaction.Repository.Accommodation?.TimeZone.Value);
    ...
    /// <summary>Unknown IANA zones fail before persistence.</summary>
    [Fact]
    public async Task AddAsync_UnknownTimeZone_FailsValidation()
    {
        var transaction = new RecordingTransaction(Creator, Plan());
        var result = await Service(transaction).AddAsync(Command() with { TimeZoneId = "Mars/Olympus" });
        Assert.Equal(AddAccommodationOutcome.ValidationFailed, result.Outcome);
    ...
    public async Task AddAsync_MismatchedOwnership_IsDenied()
    {
        ...
        var result = await Service(transaction).AddAsync(Command());
        Assert.Equal(AddAccommodationOutcome.Denied, result.Outcome);
    ...
    public async Task AddAsync_PersistenceConcurrency_ReturnsConflict()
    {
        ...
        var result = await Service(transaction).AddAsync(Command());
        Assert.Equal(AddAccommodationOutcome.Conflict, result.Outcome);
        Assert.True(transaction.Disposed);
        Assert.False(transaction.Committed);
    }

    private static AddAccommodationCommand Command() => new(
        Actor, Creator, PlanId, 1, "Rome hotel", new(2027, 1, 2), new(2027, 1, 5), "Europe/Rome");
    ...
    private static AdventurePlan Plan(CreatorId? creator = null) => new(
        ...
        PlanningStatus.Draft, new(new(2027, 1, 1), new(2027, 1, 10)), new(1, Created, Created));

    private static AccommodationAddService Service(RecordingTransaction transaction) => new(
    ...
    { public Task<CreatorMembershipSnapshot?> GetMembershipAsync(UserId userId, CreatorId creatorId, CancellationToken cancellationToken = default) => Task.FromResult<CreatorMembershipSnapshot?>(membership); }
    ...
    private sealed class Identities : IPlanningCreationIdentityGenerator
    {
        ...
        public AccommodationId NewAccommodationId() => new("accommodation_rome_01");
        public ReservationId NewReservationId() => throw new NotSupportedException();
        public AuditEventId NewAuditEventId() => new("audit_accommodation_01");
        public CorrelationId NewCorrelationId() => new("correlation_accommodation_01");
    }
    private sealed class Clock : TimeProvider { public override DateTimeOffset GetUtcNow() => Created.AddHours(1); }
    private sealed class Factory(RecordingTransaction transaction) : IPlanningTransactionFactory
    { public Task<IPlanningTransaction> BeginAsync(CreatorId creatorId, CancellationToken cancellationToken = default) => Task.FromResult<IPlanningTransaction>(transaction); }
    ...
    { public List<AuditEventIntent> Items { get; } = []; public void AddRequired(AuditEventIntent auditEvent) => Items.Add(auditEvent); }
